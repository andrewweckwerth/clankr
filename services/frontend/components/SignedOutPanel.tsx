...
export default function SignedOutPanel({ title, label, heading, description }: SignedOutPanelProps) {
  return (
    <main className="mx-auto flex min-h-[calc(100vh-5.25rem)] w-full max-w-xl items-center px-5 py-16">
      <WorkspacePanel title={title} className="w-full text-center">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-zinc-400">{label}</p>
        <h1 className="mt-4 text-3xl">{heading}</h1>
        <p className="panel-description mx-auto mt-4 max-w-md">{description}</p>
        <Link href="/sign-in" className="button-primary mt-6 inline-flex px-5 py-2.5 text-sm font-semibold">
          Sign in
        </Link>
      </WorkspacePanel>
    </main>
